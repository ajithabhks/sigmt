"""
First landing window for the Geomag2 specific operations.
...
from sigmt.gui.geomag2.geomag2_dialogs import LayoutSettingsDialog
from sigmt.gui.geomag2.geomag2_dialogs import SelectionDialog
...
from sigmt.utils.geomag2 import geomag2_utils
...
class MainWindow(QMainWindow):
    """
    Definition of the main window for Geomag2 processing
    ...
    def init_ui(self) -> None:
        ...
        self.setWindowTitle('[No Project Opened] SigMT | A Tool for '
                            'Magnetotelluric Data Processing (Geomag2)')
        ...
        if self.project_dir:
            ...
            if dialog.exec_():
                ...
                self.setWindowTitle(self.project_setup[
                                        'project_name'] +
                                    ' - SigMT | '
                                    'A Tool for Magnetotelluric Data Processing (Geomag2)')
    ...
    def open_project(self) -> None:
        ...
        try:
            ...
            if self.project_setup['interface'] == self.interface:
                ...
                QMessageBox.warning(self, "Warning", "Not a Geomag2 Project")
        except:
    ...
    def load_sites(self) -> None:
        ...
        if self.project_dir is not None:
            try:
                self.allsites = geomag2_utils.loadsites(self.project_dir)
    ...
    def site_dropdown_changed(self) -> None:
        ...
        self.localsite_meas = geomag2_utils.list_meas_folders(self.localsite_path)
        ...
        if self.remotesite is not None:
            ...
            self.remotesite_meas = geomag2_utils.list_meas_folders(self.remotesite_path)
        ...
        local_meas_paths = [os.path.join(self.project_dir, 'time_series', self.localsite, meas) for
                            meas in
                            overlapping_meas]
        [sampfreq, chopper_value] = geomag2_utils.get_sampling_frequency_from_xml(local_meas_paths)
        ...
        if self.remotesite is not None:
            remote_meas_paths = [
                ...
                overlapping_meas]
            [sampfreq, chopper_value] = geomag2_utils.get_sampling_frequency_from_xml(
                remote_meas_paths)
    ...
    def open_manual_selection_dialog(self) -> None:
        ...
        try:
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                [l_sampfreq, l_chopper_value] = geomag2_utils.get_sampling_frequency_from_xml(
                    local_meas_path)
                remote_meas_path = [
                    os.path.join(self.project_dir, 'time_series', self.remotesite, remote)]
                [r_sampfreq, r_chopper_value] = geomag2_utils.get_sampling_frequency_from_xml(
                    remote_meas_path)
        ...
        if self.remotesite is None:
            ...
            with h5py.File(self.h5file, 'w') as f:
                for local_path in local_paths:
                    self.header[f'ts_{num}'], ts_dict = geomag2_utils.read_ts(
                        meas_path=local_path,
                        project_setup=self.project_setup
                    )
                    self.xml_caldata[f'ts_{num}'] = geomag2_utils.read_calibration_from_xml(
                        meas_path=local_path
                    )
            ...
            with h5py.File(self.h5file, 'w') as f:
                for local_path, remote_path in zip(local_paths, remote_paths):
                    ...
                    header_r, ts_r = geomag2_utils.read_ts(
                    ...
                    xml_caldata_r = geomag2_utils.read_calibration_from_xml(meas_path=remote_path)
        ...
        if 'localsite' in self.procinfo and self.procinfo['localsite'] == self.localsite:
            ...
            for ts in self.header:
                ...
                bandavg = BandAveraging(
                    time_series=geomag2_utils.prepare_ts_from_h5(self.h5file, ts),
