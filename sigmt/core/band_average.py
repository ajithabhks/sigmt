...
all target frequencies and transfer functions are computed for all events.
"""
import gc
...
class BandAvg:
    ...
    def __init__(self, procinfo: dict, bandavg_msg: dict) -> None:
        ...
        #
        self.ftlist = utils.targetfreq(self.procinfo['fs'], self.procinfo['parzen_radius'],
                                       self.procinfo['fft_length'],
                                       procinfo['frequencies_per_decade'])
        self.get_channels()  # Get ts channel info, 'Ex', 'Ey', ....
        self.calibrate_electric()
        if self.procinfo['notch'] == 'on':
            self.apply_notch()
        self.detrend_ts()
        self.perform_fft()
        self.calibrate_mag(cal_data=bandavg_msg['caldata'])
        del self.ts
        gc.collect()
    ...
    def apply_notch(self) -> None:
        ...
        with ThreadPoolExecutor(max_workers=len(self.ts)) as executor:
            ...
            for channel in self.channels:
                futures[channel] = executor.submit(sp.notchfilsos, self.ts[channel],
                                                   self.procinfo['fs'],
                                                   self.procinfo['notch_frequency'])
    ...
    def perform_fft(self) -> None:
        ...
        self.xfft = {}
        for channel in self.channels:
            self.fft_freqs, self.xfft[channel] = sp.do_fft(self.ts[channel], self.procinfo['fs'],
                                                           self.procinfo['fft_length'])

    def calibrate_mag(self, cal_data) -> None:
        """
        ...
        for channel in self.mag_channels:
            ...
            if self.header[channel]['bychopper'][0] == 1:
                ...
                stat = "ChoppOff"
            calibration_data = cal_data[str(self.header[channel]['sensor_no'][0])][stat]
    ...
    def perform_bandavg(self) -> None:
        ...
        parzen_window = self._compute_parzen_window()
        ...
        if self.procinfo['processing_mode'] in ["MT + Tipper", "MT Only"]:
            self._compute_weighted_sums(parzen_window)
            self._compute_auto_cross_spectra(parzen_window)
            self._compute_transfer_functions()

        self._prepare_selection_arrays()
        print('Band averaging finished.')

    def _compute_parzen_window(self):
        """
        ...
        for i in range(self.ftlist.shape[0]):
            parzen_window[:, :, i] = stats.parzen(self.fft_freqs, float(self.ftlist[i]),
                                                  self.procinfo['parzen_radius'])
            self.dof[i] = (2 * 2 * np.sum(parzen_window[:, :, i] != 0)) - 4
            self.avgf[i] = np.sum(parzen_window[:, :, i] != 0)

        return parzen_window

    def _compute_weighted_sums(self, parzen_window):
        """
        ...
        if self.procinfo['processing_mode'] != "MT Only":
            self.bandavg_ds['hz'] = (
                ('time_window', 'frequency'),
    ...
    def _compute_auto_cross_spectra(self, parzen_window):
        """
        ...
        hz_conj: Optional[np.ndarray] = None
        if self.procinfo['processing_mode'] != "MT Only":
    ...
    def _prepare_selection_arrays(self):
        """
        Prepare selection arrays which will be used for rejection of time windows.
        """
