...
##########
Program: SigMT
A tool for the magnetotelluric time series processing
##########
...
sites, selectedsite, measid, all_meas, select_meas, proc_path = mtproc.makeprocpath(
    project_path)
# #========= Site is selected and path is created =========
# #========= Selection of remote site and setting a path ==
sites, selectedsiteR, measidR, all_measR, select_measR, proc_pathR = mtproc.makeprocpath(
    project_path)
...
procinfo = {}
procinfoR = {}
ts, procinfo['fs'], procinfo['sensor_no'], timeline, procinfo['ChoppStat'], loc = mtprocRR.ts(
    proc_path)
tsR, procinfoR['fs'], procinfoR['sensor_no'], timelineR, procinfoR['ChoppStat'], locR = mtprocRR.ts(
    proc_pathR)
# ==============
# ==============
...
# Provide frequencies to be filtered seperted by a comma, Eg:[50, 100]
procinfo['notch_freqs'] = [50, 100]
print('--------------------')
...
procinfo['nstacks'] = math.floor(
    procinfo.get('nofs')/procinfo.get('WindowLength'))
...
[cohMatrixEx, cohMatrixEy] = data_sel.performct(
    ctflag, CohThre, minpercent, ftlist, bandavg, AllcohEx, AllcohEy)
#
# ====== Polarization direction ======
pdflag = 0  # Give '1' to perform polarization direction based selection
pdlim = [-10, 10]  # Ploarization direction limit
# Use either alpha_degE (electric field) or alpha_degH (magnetic field)
alpha = alpha_degE
...
bandavg['mdmatrixEx'], bandavg['Zxx_mcd'], bandavg['Zxy_mcd'], bandavg['mahal_robustEx'] = mahaDist.mcd(
    bandavg, 'Ex', config)
bandavg['mdmatrixEy'], bandavg['Zyx_mcd'], bandavg['Zyy_mcd'], bandavg['mahal_robustEy'] = mahaDist.mcd(
    bandavg, 'Ey', config)
