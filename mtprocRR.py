...
def bandavg(ts, procinfo, tsR, procinfoR, config):
    ...
    for i in tqdm(range(np.shape(ftlist)[0])):
        ...
        for stack in (range(procinfo.get('nstacks'))):
            ...
            ExRxc[i, stack] = np.sum(
                (calEx[:, stack] * np.conj(calRx[:, stack])) * pf) / np.sum(pf)
            ExRyc[i, stack] = np.sum(
                (calEx[:, stack] * np.conj(calRy[:, stack])) * pf) / np.sum(pf)
            EyRxc[i, stack] = np.sum(
                (calEy[:, stack] * np.conj(calRx[:, stack])) * pf) / np.sum(pf)
            EyRyc[i, stack] = np.sum(
                (calEy[:, stack] * np.conj(calRy[:, stack])) * pf) / np.sum(pf)
            HxRxc[i, stack] = np.sum(
                (calHx[:, stack] * np.conj(calRx[:, stack])) * pf) / np.sum(pf)
            HxRyc[i, stack] = np.sum(
                (calHx[:, stack] * np.conj(calRy[:, stack])) * pf) / np.sum(pf)
            HyRxc[i, stack] = np.sum(
                (calHy[:, stack] * np.conj(calRx[:, stack])) * pf) / np.sum(pf)
            HyRyc[i, stack] = np.sum(
                (calHy[:, stack] * np.conj(calRy[:, stack])) * pf) / np.sum(pf)
            ...
            HzHxc[i, stack] = np.sum(
                (calHz[:, stack] * np.conj(calRx[:, stack])) * pf) / np.sum(pf)
            HzHyc[i, stack] = np.sum(
                (calHz[:, stack] * np.conj(calRy[:, stack])) * pf) / np.sum(pf)
